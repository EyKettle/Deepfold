...
export const createFileTool = tool(
  async ({ filePath, content }) => {
    const response = await invoke("create_file", {
      filePath: filePath,
      content: content,
    })
      .then(() => {
...
//   })
// })
export const initModel = async (config: ServiceConfig) => {
...
export const pushMessage = async (
  ...
  if (!systemMessageContent) throw new Error("System message content not set");
